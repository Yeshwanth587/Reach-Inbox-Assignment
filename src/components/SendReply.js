...
const SendReply = ({ ColorMode, handleCancel, singleMail }) => {
  ...
  useEffect(() => {
    ...
    setFormData({
      ...formData, toName: singleMail.toName, fromName: singleMail.fromEmail,
  ...
  const handlesubmit = () => {
    ...
    postMailMasseges(singleMail.threadId, formData).then((res) => {
  ...
  return (
    <div className='flex flex-col w-full h-full '>
      <div>
        <div className={`h-10 flex justify-between px-8 py-2 text-[16px]  ${ColorMode ? 'bg-[#23272C]' : 'bg-white'}`}>
          <p>Reply</p>
        ...
        <div className={`text-[12px] h-10 flex pt-2 px-8 py-2 h-7   items-center gap-2`} >
          ...
          <input type="text" placeholder='' value={formData.to} className={`${ColorMode ? 'bg-[#141517]' : "bg-white"} outline-none`} onChange={(e) => setFormData({ ...formData, to: e.target.value })} />
        </div>
        <hr className='border-t border-gray-700' />
        <div className={`text-[12px] h-10 flex pt-2 px-8 py-2   h-7  items-center gap-2`} >
          <p className='text-gray-400'>From :- </p>
          <input type="text" placeholder='' value={formData.from} className={`${ColorMode ? 'bg-[#141517]' : "bg-white"} outline-none`} onChange={(e) => setFormData({ ...formData, from: e.target.value })} />
        </div>
        <hr className='border-t border-gray-700' />
        <div className={`text-[12px] h-10 flex pt-2 px-8 py-2  h-7 items-center gap-2`} >
          <p className='text-gray-400'>Subject :- </p>
          <input type="text" placeholder='' value={formData.subject} className={`${ColorMode ? 'bg-[#141517]' : "bg-white"} outline-none`} onChange={(e) => setFormData({ ...formData, subject: e.target.value })} />
        </div>
        <hr className='border-t border-gray-700' />
        <div className={`text-[12px] p-2 text-left`}>
          <textarea placeholder='Hello John...' value={formData.body} className={`ml-6   h-[250px] w-[500px] outline-none ${ColorMode ? 'bg-[#141517]' : 'bg-white'}`} onChange={(e) => setFormData({ ...formData, body: e.target.value })} />
        </div>
      </div>
