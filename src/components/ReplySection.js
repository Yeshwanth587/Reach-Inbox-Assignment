...
const ReplySection = ({ ColorMode, singleMail }) => {
            ...
            <div className=' relative w-[800px] flex justify-between flex-col ' >
                <div className='flex h-[70px] border border-gray-700 py-3 pl-4'>
                    ...
                    <div className='flex justify-end mr-6  gap-4 h-8 my-2 w-full text-right'>
                        <div className={`flex justify-center gap-2 p-2 items-center border border-gray-700 rounded ${ColorMode ? 'bg-[#222426]' : 'bg-[#fff]'}`} >
                            <p className='w-3 h-3 rounded-3xl bg-yellow-600'></p>
                            <p className='text-[12px]'>Meeting Completed</p>
                            <ChevronDown />
                        </div>
                        <div className={` flex justify-center p-2 items-center border border-gray-700 rounded ${ColorMode ? 'bg-[#222426]' : 'bg-[#fff]'}`} >
                ...
                <div className='py-3 pl-4 '>
                    {
                        singleMail?.length > 0 && singleMail?.map((item) => {
                            return <SingleMail ColorMode={ColorMode} {...item} key={item.id} />
                        })
                    }
                    <h2 className='text-[10px] mt-2 mb-3'>Today</h2>
                </div>
                {
                    showReply && <div className={`absolute mt-[134px] w-[768px] ml-4 rounded-2xl  z-10  overflow-hidden  h-[450px] ${ColorMode ? 'bg-[#141517]' : 'bg-white'}  border border-gray-700`}>
                        <SendReply ColorMode={ColorMode} singleMail={singleMail[0]} handleCancel={() => setShowReply(!showReply)} />
                    </div>
                }
            </div>
